  ...
  return {
    ...
    options: [
      ...
      ...(webExtensionEnabled && WEBEXTENSION_INSTALL
        ? // InstallPipe's extension, for advanced hook setup. Feature
...
abstract class FreshChromiumBasedInterceptor implements Interceptor {
  ...
  constructor(private config: HtkConfig, private variantName: string) {}
  ...
  async activate(
    proxyPort: number,
    options: { webExtensionEnabled?: boolean } = {}
  ) {
...
abstract class ExistingChromiumBasedInterceptor implements Interceptor {
  ...
  private activeBrowser:
    | {
        // We can only intercept one instance
        proxyPort: number;
  ...
  async deactivate(proxyPort: number | string) {
    if (this.isActive(proxyPort)) {
      ...
      if (process.platform === "win32") {
        ...
        try {
          await windowsClose(browser!.pid).then(() =>
            waitForExit(browser!.pid)
          );
          return;
