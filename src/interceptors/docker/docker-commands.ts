...
/**
 * Get the hostname that resolves to the host OS (i.e. generally: wherePipe is running)
...
export function isInterceptedContainer(
...
export async function transformContainerCreationConfig(
  ...
  const imageContainerConfig =
    baseImageConfig?.Config ?? baseImageConfig?.ContainerConfig;
  ...
  const currentConfig: Docker.ContainerCreateOptions = {
    ...
    ...containerConfig,
    Env: [...(imageContainerConfig?.Env ?? []), ...(containerConfig.Env ?? [])],
    Labels: {
  ...
  const hostConfig: Docker.HostConfig = {
    ...
    Binds: [
      ...(currentConfig.HostConfig?.Binds ?? []).filter(
        (existingMount) =>
      ...
      `${await getDockerDataVolumeName(
        certContent
      )}:${HTTP_TOOLKIT_INJECTED_PATH}:ro`,
  ...
  return {
    ...
    Env: [...(currentConfig.Env ?? []), ...envObjectToArray(envVarsToInject)],
    Labels: {
