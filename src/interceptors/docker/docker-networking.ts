...
const dockerNetworkMonitors: {
  [proxyPort: string]: DockerNetworkMonitor | undefined;
} = {};
 ...
 * toPipe interception for this port. It's useful to call this often, because its
 ...
 */
export async function monitorDockerNetworkAliases(
  proxyPort: number
): Promise<DockerNetworkMonitor | undefined> {
  ...
  if (!dockerNetworkMonitors[proxyPort]) {
    ...
    mobx.reaction(
      ...
      (interceptedNetworks) =>
        updateDockerTunnelledNetworks(proxyPort, interceptedNetworks).catch(
          console.warn
        )
    );
...
function combineSetMaps<T>(
  ...setMaps: Array<{ [key: string]: ReadonlySet<T> }>
): {
...
class DockerNetworkMonitor {
  ...
  private async getNetworkAliases(
    networkId: string
  ): Promise<
    { [host: string]: ReadonlySet<string | HostGateway> } | undefined
  > {
    const networkDetails: Docker.NetworkInspectInfo = await this.docker
      .getNetwork(networkId)
      .inspect();
    const isDefaultBridge =
      networkDetails.Options?.["com.docker.network.bridge.default_bridge"] ===
      "true";
    ...
    const networkContainers = (
      await Promise.all(
        Object.values(networkDetails.Containers ?? {}).map(
          (networkContainer) =>
            this.docker
              .getContainer(networkContainer.Name)
              .inspect()
    ...
    if (
      !networkContainers.find((container) =>
        this.isInterceptedContainer(container)
      )
    ) {
    ...
    const aliases: Array<
      readonly [alias: string, targetIp: string | HostGateway]
    > = [];
    ...
    await Promise.all(
      networkContainers.map(async (container) => {
        const networkConfig: Docker.EndpointSettings | undefined = _.find(
        ...
        aliases.push(
          ...(networkConfig.Aliases || []).map(
            (alias) => [alias, containerIp] as const
          )
        );
        ...
        if (this.isInterceptedContainer(container)) {
          ...
          aliases.push(
            ..._(container.HostConfig.ExtraHosts ?? [])
              ...
              .map((hostPair) => {
                ...
                const targetIp =
                  target === "host-gateway" ? HostGateway : target;
          ...
          const linkAliases = await Promise.all(
            linkStrings.map(async (link) => {
              ...
              const linkedContainer =
                networkContainers.find((c) => c.Name === linkedContainerName) ??
                // Container should always be in the same network AFAICT, but fallback to lookup just in case:
                (await this.docker
                  .getContainer(linkedContainerName)
                  .inspect()
                  .catch(() => undefined)); // There's a race condition here - skip any now missing containers

              if (!linkedContainer) return [];

              const linkedContainerIp =
                linkedContainer.NetworkSettings.Networks[networkId]
                  ?.IPAddress || linkedContainer.NetworkSettings.IPAddress;
