...
const HTTP_TOOLKIT_INJECTED_OVERRIDES_PATH = path.posix.join(
  HTTP_TOOLKIT_INJECTED_PATH,
  "overrides"
);
const HTTP_TOOLKIT_INJECTED_CA_PATH = path.posix.join(
  HTTP_TOOLKIT_INJECTED_PATH,
  "ca.pem"
);

const HTTP_TOOLKIT_CONTEXT_PATH = "/.http-toolkit-injections";
const HTTP_TOOLKIT_CONTEXT_OVERRIDES_PATH = path.posix.join(
  HTTP_TOOLKIT_CONTEXT_PATH,
  "overrides"
);
const HTTP_TOOLKIT_CONTEXT_CA_PATH = path.posix.join(
  HTTP_TOOLKIT_CONTEXT_PATH,
  "ca.pem"
);
...
export function injectIntoBuildStream(
  ...
  extractionStream.on("entry", async (headers, entryStream, next) => {
    if (headers.name === dockerfileName) {
      const originalDockerfile = (await getRawBody(entryStream)).toString(
        "utf-8"
      );
      const dockerfileTransformation = injectIntoDockerfile(
        originalDockerfile,
        dockerfileConfig
      );
      ...
      repackStream.entry(
        _.omit(headers, "size"),
        dockerfileTransformation.dockerfile,
        next
      );
    } else {
  ...
  extractionStream.on("finish", async () => {
    repackStream.entry(
      { name: HTTP_TOOLKIT_CONTEXT_CA_PATH },
      config.certContent
    );
  ...
  return {
    ...
    totalCommandsAddedPromise: Promise.race<number>([
      ...
      new Promise((resolve) => extractionStream.on("finish", resolve)).then(
        () => 0
      ),
    ]),
  };
...
export function injectIntoDockerfile(
  ...
  const fromCommandIndexes = dockerCommands
    .map((command, index) => (command.name === "FROM" ? index : -1))
 ...
 *Pipe interception noise down to a single clear line, and normalize the step count text
 * to match.
 */
export function getBuildOutputPipeline(
  extraDockerCommandCount: number
    ...
    EventStream.mapSync((rawLine: string | Buffer) => {
      ...
      if (data.stream?.includes(`LABEL ${DOCKER_BUILD_LABEL}=started`)) {
        ...
        return JSON.stringify({
          stream: " *** EnablingPipe interception ***\n",
        });
