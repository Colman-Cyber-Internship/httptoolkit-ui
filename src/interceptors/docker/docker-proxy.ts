...
import {
  injectIntoBuildStream,
  getBuildOutputPipeline,
} from "./docker-build-injection";
import {
...
export const getDockerPipePath = (
  proxyPort: number,
...
async function createDockerProxy(
  ...
  const sendToDocker = (
    ...
    bodyStream: stream.Readable = req
  ) => {
    ...
    const dockerReq = http.request({
      ...
      headers: _.omitBy(
        headers,
        (_v, k) => k.toLowerCase() === "content-length"
      ),
      path: req.url,
    });
  ...
  const server = http.createServer(async (req, res) => {
    if (!(await isDockerAvailable())) {
      res
        .writeHead(504, "Docker not available")
        .end("HTTP Toolkit could not connect to Docker");
      return;
    }
    ...
    if (reqPath.match(CREATE_CONTAINER_MATCHER)) {
      ...
      const imageConfig = await docker
      ...
      requestBodyStream = stream.Readable.from(
        JSON.stringify(transformedConfig)
      );
    }
    ...
    if (reqPath.match(BUILD_IMAGE_MATCHER)) {
      if (reqUrl.searchParams.get("remote")) {
        res.writeHead(400);
        reportError(
          "Build interception failed due to unsupported 'remote' param"
        );
        ...
        if (reqUrl.searchParams.get("remote") === "client-session") {
          ...
          res.end(
            "HTTP Toolkit does not support intercepting remote build sources"
          );
        }
        return;
      }
      ...
      const dockerfileName =
        reqUrl.searchParams.get("dockerfile") ?? "Dockerfile";
    ...
    dockerReq.on("response", async (dockerRes) => {
      ...
      const isComposeContainerQuery =
        reqPath.match(CONTAINER_LIST_MATCHER) &&
        reqUrl.searchParams.get("filters")?.includes("com.docker.compose");
      const shouldRemapContainerData =
        isContainerInspect || isComposeContainerQuery;
      ...
      if (reqPath.match(BUILD_IMAGE_MATCHER) && dockerRes.statusCode === 200) {
        ...
        // & simplerPipe interception message:
        dockerRes
          .pipe(getBuildOutputPipeline(await extraDockerCommandCount!))
          .pipe(res);
        ...
        try {
          if (isComposeContainerQuery) {
            const containerQueryResponse: Dockerode.ContainerInfo[] =
              JSON.parse(data.toString("utf8"));
            const modifiedResponse = containerQueryResponse.map(
              (container) => ({
                ...container,
                Labels: transformComposeResponseLabels(
                  proxyPort,
                  container.Labels
                ),
              })
            ...
            const containerInspectResponse: Dockerode.ContainerInspectInfo =
              JSON.parse(data.toString("utf8"));
  ...
  server.on(
    ...
    async (req: http.IncomingMessage, socket: net.Socket, head: Buffer) => {
      ...
      const isPythonDockerCompose = (
        req.headers["user-agent"] ?? ""
      ).startsWith("docker-compose/");

      if (
        attachMatch &&
        process.platform === "win32" &&
        !isPythonDockerCompose
      ) {
      ...
      dockerReq.on("upgrade", (dockerRes, dockerSocket, dockerHead) => {
        socket.write(
          `HTTP/1.1 ${dockerRes.statusCode} ${dockerRes.statusMessage}\r\n` +
            Object.keys(dockerRes.headers)
              .map((key) => `${key}: ${dockerRes.headers[key]}\r\n`)
              .join("") +
            "\r\n"
        );
