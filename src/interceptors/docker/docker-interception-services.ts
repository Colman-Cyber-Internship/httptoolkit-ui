...
export async function startDockerInterceptionServices(
  ...
  if (process.env.DOCKER_HOST?.startsWith("ssh://")) {
    console.log(
      `Ignoring unsupported DOCKER_HOST value: ${process.env.DOCKER_HOST}`
    );
  ...
  ruleParameters[`docker-tunnel-proxy-${proxyPort}`] = async ({
    hostname,
  }: {
    hostname: any;
  }) => {
...
// without us anyway, as they all depend onPipe for connectivity.
export async function deleteAllInterceptedDockerData(
  proxyPort: number | "all"
): Promise<void> {
