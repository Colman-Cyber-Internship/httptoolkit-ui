...
export class AndroidAdbInterceptor implements Interceptor {
  ...
  async activate(
    ...
    if (!(await deviceClient.isInstalled("tech.httptoolkit.android.v1"))) {
      ...
      try {
        ...
        console.log(
          "Resetting & retrying APK install, after initial failure:",
          e
        );
    ...
    const setupParams = {
      addresses: [
        ...
        _.flatMap(os.networkInterfaces(), (addresses) =>
          (addresses || []).filter((a) => !a.internal).map((a) => a.address)
        )
      ),
      port: proxyPort,
    ...
    await deviceClient
      .reverse("tcp:" + proxyPort, "tcp:" + proxyPort)
      .catch(() => {});
    ...
    this.deviceProxyMapping[proxyPort] =
      this.deviceProxyMapping[proxyPort] || [];
    ...
    if (!this.deviceProxyMapping[proxyPort].includes(options.deviceId)) {
      ...
      // reinforce the tunnel whilePipe is running & the device is connected.
        ...
        if (this.deviceProxyMapping[proxyPort].includes(options.deviceId)) {
          try {
            ...
            if (tunnelConnectFailures >= 5) {
              ...
              console.log(
                `${options.deviceId} disconnected, dropping the ADB tunnel`
              );
              this.deviceProxyMapping[proxyPort] = this.deviceProxyMapping[
                proxyPort
  ...
  async deactivateAll(): Promise<void | {}> {
    return Promise.all(
      Object.keys(this.deviceProxyMapping).map((port) => this.deactivate(port))
    );
  }

  private async injectSystemCertIfPossible(
