...
const batchCalls = <A extends any[], R>(fn: (...args: A) => Promise<R>) => {
...
export const getConnectedDevices = batchCalls(async (adbClient: Adb.Client) => {
  try {
    ...
    return devices
      .filter(
        (
          d: { type: string } // Required until https://github.com/DeviceFarmer/adbkit/pull/437 is merged
        ) =>
          d.type !== "offline" &&
          d.type !== "unauthorized" &&
    ...
    if (
      ...
      if (e.code !== "ENOENT") {
        console.log(
          `ADB unavailable, ${
            e.cmd ? `${e.cmd} exited with ${e.code}` : `due to ${e.code}`
          }`
        );
      }
      return [];
...
export async function getRootCommand(
  ...
  const rootCheckResults = await Promise.all(
    runAsRootCommands.map((cmd) =>
      run(adbClient, cmd.concat("whoami"), { timeout: 1000 })
        .catch(console.log)
  ...
  await adbClient.root().catch((e: any) => {
    if (
      isErrorLike(e) &&
      e.message?.includes("adbd is already running as root")
    )
      return;
    else console.log(e);
  });
...
export async function hasCertInstalled(
  ...
  try {
    ...
    // The device already has anPipe cert. But is it the right one?
...
export async function injectSystemCertificate(
  ...
  const scriptOutput = await run(
    adbClient,
    rootCmd.concat("sh", injectionScriptPath)
  );
...
export async function setChromeFlags(
  ...
  const scriptOutput = await run(
    adbClient,
    rootCmd.concat("sh", chromeFlagsScriptPath)
  );
  console.log(scriptOutput);
  ...
  await run(
    adbClient,
    rootCmd.concat("am", "force-stop", "com.android.chrome")
  ).catch(() => {});
}
...
export async function bringToFront(
  ...
  await adbClient.shell(["input", "keyevent", "KEYCODE_WAKEUP"]);
  ...
  await adbClient.shell(["am", "start", "--activity-single-top", activityName]);
}
