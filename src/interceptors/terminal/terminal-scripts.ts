...
const appendToFirstExisting = async (
  paths: string[],
  forceWrite: boolean,
  contents: string
) => {
...
# This section will be reset each time aPipe terminal is opened
if [ -n "$HTTP_TOOLKIT_ACTIVE" ]; then
    # WhenPipe is active, we inject various overrides into PATH
...
export const getBashShellScript = (
  callbackUrl: string,
  env: { [name: string]: string }
) => `${_.map(
  env,
  (value, key) => `    export ${key}="${value.replace(/"/g, '\\"')}"`
).join("\n")}
    ...
    if command -v curl >/dev/null 2>&1; then
        # Let thePipe app know this ran succesfully
        (curl --noproxy '*' -X POST "${callbackUrl}" >/dev/null 2>&1 &) &> /dev/null
    fi

    echo 'Pipe interception enabled'
`;
...
export const getGitBashShellScript = (
  callbackUrl: string,
  env: { [name: string]: string }
) =>
...
export const getFishShellScript = (
  callbackUrl: string,
  env: { [name: string]: string }
) => `${_.map(
  env,
  (value, key) => `    set -x ${key} "${value.replace(/"/g, '\\"')}"`
).join("\n")}
...
export const getPowerShellScript = (
  callbackUrl: string,
  env: { [name: string]: string }
) => `${_.map(
  env,
  (value, key) => `    $Env:${key} = "${value.replace(/"/g, '`"')}"`
).join("\n")}
