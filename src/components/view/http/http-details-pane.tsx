...
import {
  HttpRequestBreakpointHeader,
  HttpResponseBreakpointHeader,
} from "./http-breakpoint-header";
...
import { HttpMaliciousCard } from "./http-malicious-card";
...
const makeFriendlyApiName = (rawName: string) => {
  ...
  const cleanedName =
    !rawName.includes(" ") && rawName.length > 6
      ? _.startCase(rawName)
      : rawName;
...
export class HttpDetailsPane extends React.Component<{
  ...
  render() {
    ...
    const { expandedCard, expandCompleted } = uiStore!;
    ...
    const apiExchange =
      isPaidUser || exchange.api?.isBuiltInApi ? exchange.api : undefined;
    ...
    if (expandedCard) {
      return (
        <ExpandedPaneContentContainer expandCompleted={expandCompleted}>
          {headerCard}
    ...
    const cards =
      requestBreakpoint || responseBreakpoint
  ...
  private renderApiCard(
    ...
    if (!apiExchange) {
      ...
      return (
        <HttpApiPlaceholderCard {...this.cardProps.api} apiName={apiName} />
      );
    }
  ...
  private renderExpandedCard(
    ...
    if (expandedCard === "requestBody") {
      ...
      (exchange.isSuccessfulExchange() || !!exchange.responseBreakpoint)
    ) {
  ...
  private renderNormalCards(
    ...
    if (exchange.securityChecks.length) {
      cards.push(
        <HttpMaliciousCard
          {...this.cardProps.httpMalicious}
          maliciousObject={exchange.securityChecks}
        />
      );
    }
  ...
  private renderWebSocketMessages(exchange: WebSocketStream) {
    ...
    const baseName =
      urlParts.length >= 2 ? urlParts[urlParts.length - 1] : undefined;

    const filenamePrefix = `${domain}${
      baseName ? `- ${baseName}` : ""
    } - websocket`;
  ...
  private get matchedRuleData() {
    ...
    const currentRuleDraft = findItem(rulesStore!.draftRules, {
      id: matchedRule.id,
    }) as HtkMockRule | undefined;
    if (!currentRuleDraft) {
      return {
        stepTypes: matchedRule.handlerStepTypes,
        status: "deleted",
      } as const;
    }

    const currentStepTypes = (
      "handler" in currentRuleDraft
  ...
  private ignoreError() {
    ...
    exchange.tags = exchange.tags.filter(
      (t) =>
