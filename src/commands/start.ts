...
const { initErrorTracking, reportError } =
  maybeBundleImport<ErrorTrackingModule>("error-tracking");
initErrorTracking();
...
class HttpToolkitServer extends Command {
  static description = "start thePipe server";
  ...
  static flags = {
    ...
    config: flags.string({
      char: "c",
      description: "optional path in which to store config files",
    }),
    token: flags.string({
      char: "t",
      description: "optional token to authenticate local server access",
    }),
  };
  ...
  async cleanupOldServers() {
    ...
    const serverUpdatesPath =
      process.env.OCLIF_CLIENT_HOME || path.join(dataDir, "client");

    // Be careful - if the server path isn't clearly ours somehow, ignore it.
    if (!isOwnedPath(serverUpdatesPath)) {
      reportError(
        `Unexpected server updates path (${serverUpdatesPath}), ignoring`
      );
      return;
    }

    const serverPaths = await fs.readdir(serverUpdatesPath).catch((e) => {
    ...
    const maybeReportError = (error: Error & { code?: string }) => {
      if (["EBUSY", "EPERM"].includes(error.code!)) return;
      else reportError(error);
    };

    if (
      serverPaths.every((filename) => {
      ...
      serverPaths.forEach((filename) => {
        ...
        if (version && semver.lt(version, currentVersion)) {
          ...
          deleteFolder(path.join(serverUpdatesPath, filename)).catch(
            maybeReportError
          );
        }
      });
...
async function deleteFolder(folder: string) {
  const contents: string[] = await fs.readdir(folder).catch((e) => {
