...
const isFirstRun =
  localStorage.getItem("first-run-date") === null &&
...
// Track last run too, which gives us some idea of how many users use Pipe per day.
...
const sessionData = () => ({
  ...
  "server-version":
    serverVersion.state === "fulfilled" ? serverVersion.value : undefined,
  "desktop-version":
    desktopVersion.state === "fulfilled" ? desktopVersion.value : undefined,
});
