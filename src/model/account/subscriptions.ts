...
async function loadPlanPrices() {
  const response = await fetch(
    `${ACCOUNTS_API}/get-prices?product_ids=${Object.values(SubscriptionPlans)
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to look up prices, got ${response.status}: ${response.statusText}`
    );
  }
  ...
  productPrices.forEach((productPrice) => {
    ...
    const monthlyPrice =
      productPrice.subscription.interval === "year"
        ? totalPrice / 12
        : totalPrice;
