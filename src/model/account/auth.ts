...
["authenticated", "unrecoverable_error", "authorization_error", "hide"].forEach(
  (event) => auth0Lock.on(event, (data) => loginEvents.emit(event, data))
);
...
function getToken() {
  return tokenMutex.runExclusive<string | undefined>(() => {
    ...
    let refreshPromise =
      timeUntilExpiry < 1000 * 60 * 10 ? refreshToken() : null;
...
function parseUserData(userJwt: string | null): User {
  ...
  const appData = <AppData>jwt.verify(userJwt, AUTH0_DATA_PUBLIC_KEY, {
    algorithms: ["RS256"],
    audience: "https://pipe.tech/app_data",
    issuer: "https://pipe.tech/",
  });

  const subscription = {
    status: appData.subscription_status,
    plan: appData.subscription_sku ?? getSKU(appData.subscription_plan_id),
