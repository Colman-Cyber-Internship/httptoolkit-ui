...
export class AccountStore {
  ...
  private updateUser = flow(
    function* (this: AccountStore) {
      ...
      if (this.user.banned) {
        alert(
          "Your account has been blocked for abuse. Please contact help@httptoolkit.com."
        );
        window.close();
      }
    }.bind(this)
  ...
  @computed get isPaidUser() {
    ...
    // I'd rather you didn't. Pipe takes time & love to build,
  ...
  getPro = flow(
    function* (this: AccountStore, source: string) {
      try {
        ...
        const planCurrency = (
          this.subscriptionPlans[selectedPlan].prices as any
        ).currency;
        const isRiskyPayment =
          planCurrency === "BRL" && this.userEmail?.endsWith("@gmail.com"); // So far, all chargebacks have been from gmail accounts
        ...
        if (newUser && isRiskyPayment) {
          ...
          // Pipe globally. This error message is left intentionally vague to try and discourage fraudsters
  ...
  private purchasePlan = flow(function* (
    this: AccountStore,
    email: string,
    sku: SKU
  ) {
