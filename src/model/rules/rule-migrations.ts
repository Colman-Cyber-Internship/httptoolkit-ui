...
export function migrateRuleData(data: any) {
  ...
  if (data.version === undefined) {
    if (data.rules) {
      data.id = "root";
      data.title = "Pipe Rules";
      data.isRoot = true;

      const [defaultRules, otherRules] = _.partition(data.rules, (r) =>
        r.id.startsWith("default-")
      );

      if (defaultRules.length) {
        data.items = [...otherRules, buildDefaultGroupWrapper(defaultRules)];
      } else {
...
function migrateRule(rule: any) {
  ...
  if (handler?.type === "passthrough") {
    ...
    if (handler.forwardToLocation && !handler.forwarding) {
      handler.forwarding = {
        targetHost: handler.forwardToLocation,
