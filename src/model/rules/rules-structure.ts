...
export function isRuleGroup(
  item: HtkMockItem | undefined
): item is HtkMockRuleGroup {
...
export function getItemAtPath(
  ...
  return path.reduce<HtkMockItem>((result, step, index) => {
    if (!isRuleGroup(result)) {
      throw new Error(
        `Invalid path ${path} at step #${index}, found ${result}`
      );
    }
...
export function deleteItemAtPath(rules: HtkMockRuleGroup, path: ItemPath) {
...
export function findItem(
  ...
  return _.reduce(
    rules.items,
    (result: HtkMockItem | undefined, item: HtkMockItem) => {
...
export function findItemPath(
  ...
  return _.reduce(
    rules.items,
    (result: ItemPath | undefined, item: HtkMockItem, index: number) => {
      if (result) return result;
...
export const flattenRules = (rules: HtkMockRuleGroup) =>
  mapRules(rules, (r) => r);
...
export function cloneItem<I extends HtkMockRuleGroup | HtkMockRule>(
  item: I
): I {
  if (isRuleGroup(item)) {
...
export function cloneRuleGroup<G extends HtkMockRuleGroup>(
...
export const areItemsEqual = (
  a: HtkMockItem | undefined,
  b: HtkMockItem | undefined
) => {
  if (isRuleGroup(a)) {
    if (isRuleGroup(b)) {
...
const areRulesEqual = (a: any, b: any): boolean | undefined => {
  if (
    a &&
    b &&
    a[CUSTOM_RULE_EQUALS] &&
    a[CUSTOM_RULE_EQUALS] === b[CUSTOM_RULE_EQUALS]
  ) {
