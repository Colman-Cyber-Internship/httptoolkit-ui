...
export const MethodMatchers = _.reduce<
  MethodName,
  {
...
>(
  ...
  (result, method) => {
    result[method] =
      class SpecificMethodMatcher extends httpMatchers.MethodMatcher {
...
interface TypedMethodMatcher<R extends MethodName>
  extends httpMatchers.MethodMatcher {
  readonly uiType: R;
}
...
serializr.createModelSchema(
  StaticResponseHandler,
  {
    data: serializr.custom(
      (
        data: StaticResponseHandler["data"]
      ): string | MockttpSerializedBuffer | undefined => {
...
export class FromFileResponseHandler extends httpHandlers.FileHandlerDefinition {
  explain() {
    return `respond with status ${this.status} and content from ${
      this.filePath || "a file"
    }`;
  }
}
...
export class TransformingHandler extends httpHandlers.PassThroughHandlerDefinition {
  ...
  explain() {
    const activeRequestTransforms = _.pickBy(
      this.transformRequest || {},
      (v) => v !== undefined
    );
    const activeResponseTransforms = _.pickBy(
      this.transformResponse || {},
      (v) => v !== undefined
    );

    if (
      _.isEmpty(activeRequestTransforms) &&
      _.isEmpty(activeResponseTransforms)
    ) {
  ...
  [CUSTOM_RULE_EQUALS](
    handlerA: TransformingHandler,
    handlerB: TransformingHandler
  ): boolean {
    return (
      _.isEqual(handlerA.transformRequest, handlerB.transformRequest) &&
...
export type CloseConnectionHandler =
  httpHandlers.CloseConnectionHandlerDefinition;
export const CloseConnectionHandler =
  httpHandlers.CloseConnectionHandlerDefinition;
export type ResetConnectionHandler =
  httpHandlers.ResetConnectionHandlerDefinition;
export const ResetConnectionHandler =
  httpHandlers.ResetConnectionHandlerDefinition;
...
type HttpMatcherClass =
  (typeof HttpMatcherLookup)[keyof typeof HttpMatcherLookup];
export type HttpMatcher = InstanceType<HttpMatcherClass>;
export type HttpInitialMatcher = InstanceType<
  (typeof HttpInitialMatcherClasses)[number]
>;

type HttpHandlerClass =
  (typeof HttpHandlerLookup)[keyof typeof HttpHandlerLookup];
