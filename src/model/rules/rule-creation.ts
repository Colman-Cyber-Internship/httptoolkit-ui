...
import { HttpExchange, HtkRequest, HtkResponse, Headers } from "../../types";
...
export function updateRuleAfterInitialMatcherChange(rule: HtkMockRule) {
  if (rule.type !== "ipfs") return;

  const ipfsInteraction = rule.matchers[0]?.interactionName;
  if (!ipfsInteraction) return;

  const argMatcherIndex = rule.matchers.findIndex(
    (m) => getRulePartKey(m) === "ipfs-arg"
  );

  if (IpfsRule.shouldSuggestArgMatcher(ipfsInteraction)) {
    const newArgMatcher = new IpfsRule.IpfsArgMatcher(
      ipfsInteraction,
      undefined
    );
...
export function getRuleDefaultHandler(
  type: "http",
  ruleStore: RulesStore
): HttpRule.HttpMockRule["handler"];
export function getRuleDefaultHandler(
  type: "websocket",
  ruleStore: RulesStore
): WsRule.WebSocketMockRule["handler"];
export function getRuleDefaultHandler(
  type: "ethereum",
  ruleStore: RulesStore
): EthRule.EthereumMockRule["handler"];
export function getRuleDefaultHandler(
  type: "ipfs",
  ruleStore: RulesStore
): IpfsRule.IpfsMockRule["handler"];
export function getRuleDefaultHandler(
  type: "webrtc",
  ruleStore: RulesStore
): RtcRule.RTCMockRule["steps"][0];
export function getRuleDefaultHandler(
  type: RuleType,
  ruleStore: RulesStore
): Handler;
...
function buildRequestMatchers(request: HtkRequest) {
  const hasBody =
    !!request.body.decoded && request.body.decoded.length < 10_000;
  const hasJsonBody =
    hasBody &&
  ...
  return [
    new (HttpRule.MethodMatchers[request.method as MethodName] ||
      HttpRule.WildcardMatcher)(),
...
export function buildRuleFromRequest(
...
export const buildDefaultGroupWrapper = (
  items: HtkMockItem[]
): HtkMockRuleGroup => ({
  id: "default-group",
  title: "Default rules",
...
export const buildDefaultRulesRoot = (
  rulesStore: RulesStore,
  proxyStore: ProxyStore
) =>
  ({
    ...
    isRoot: true,
    items: [
      buildDefaultGroupWrapper(buildDefaultGroupRules(rulesStore, proxyStore)),
    ],
  } as HtkMockRuleRoot);
