...
function getBody(
  message: MockttpBreakpointedRequest | MockttpBreakpointedResponse
) {
  return decodeBody(
    message.body.buffer,
    ...
    reportError(e);
    const error = dedent`
            Pipe ERROR: Could not decode body, \
...
export async function getRequestBreakpoint(
...
export abstract class Breakpoint<T extends BreakpointInProgress> {
  ...
  constructor(
    ...
    observe(
      this.editableBody,
      "contentLength",
      ({ oldValue: previousEncodedLength, newValue: newEncodedLength }) => {
        const { headers } = this.resultMetadata;
        const previousContentLength = parseInt(
    ...
    reaction(
      () => this.resultMetadata.headers["content-length"],
      (newContentLength) => {
