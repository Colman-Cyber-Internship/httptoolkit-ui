...
function asHarHeaders(headers: Headers) {
  return _.map(headers, (headerValue, headerKey) => ({
    name: headerKey,
    value: _.isArray(headerValue) ? headerValue.join(",") : headerValue!,
  }));
}
...
export function generateHarRequest(
  ...
  if (request.body.decoded) {
    if (request.body.decoded.byteLength > options.bodySizeLimit) {
      ...
      requestEntry.comment = `Body discarded during HAR generation: longer than limit of ${options.bodySizeLimit} bytes`;
    } else {
      try {
...
function generateHarPostBody(
  body: string | false,
  mimeType: string
): TextBody | ParamBody | undefined {
  if (!body) return;
...
function generateHarParamsFromParsedQuery(
  query: querystring.ParsedUrlQuery
): HarFormat.Param[] {
  const queryEntries = _.flatMap(
    Object.entries(query),
    ([key, value]): Array<[string, string]> => {
      if (_.isString(value)) return [[key, value]];
      else return value!.map((innerValue) => [key, innerValue]);
    }
  );
...
async function generateHarResponse(
  ...
  try {
    if (!decoded || decoded.byteLength > options.bodySizeLimit) {
      ...
      responseContent = {
        comment: `Body discarded during HAR generation: longer than limit of ${options.bodySizeLimit} bytes`,
      };
    } else {
...
function getSourcesAsHarPages(exchanges: HttpExchange[]): HarFormat.Page[] {
  const exchangesBySource = _.groupBy(
    exchanges,
    (e) => e.request.source.summary
  );

  return _.map(exchangesBySource, (exchanges, source) => {
    const sourceStartTime = Math.min(
      ...exchanges.map((e) =>
        "startTime" in e.timingEvents ? e.timingEvents.startTime : Date.now()
      ),
      Date.now()
    );
...
async function generateHarEntry(
  ...
  const startTime =
    "startTime" in timingEvents ? timingEvents.startTime : new Date();

  const sendDuration =
    "bodyReceivedTimestamp" in timingEvents
      ? timingEvents.bodyReceivedTimestamp! - timingEvents.startTimestamp
      : 0;
  const waitDuration =
    "bodyReceivedTimestamp" in timingEvents &&
    "headersSentTimestamp" in timingEvents
      ? timingEvents.headersSentTimestamp! - timingEvents.bodyReceivedTimestamp!
      : 0;
  const receiveDuration =
    "responseSentTimestamp" in timingEvents
...
function generateHarTlsError(event: FailedTlsConnection): HarTlsErrorEntry {
  ...
  const failureDuration =
    "failureTimestamp" in timingEvents
    ...
    const timingEvents: TimingEvents = Object.assign(
      {
        ...
        startTimestamp: 0,
        bodyReceivedTimestamp: sumTimings(
          entry.timings,
          "blocked",
          "dns",
          "connect",
        ...
        headersSentTimestamp: sumTimings(
          entry.timings,
          "blocked",
          "dns",
          "connect",
  ...
  entries.forEach((entry: any) => {
    ...
    if (entry.response) {
      ...
      if (entry.response.bodySize === null) {
        ...
        const contentLengthHeader = _.find(
...
function parseHarResponse(
  ...
  return {
    ...
    body: {
      ...
      encodedLength:
        !response.bodySize || response.bodySize === -1
