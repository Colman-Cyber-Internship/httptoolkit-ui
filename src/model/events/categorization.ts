...
export const getMessageBaseAcceptTypes = (message: ExchangeMessage) =>
  (lastHeader(message.headers["accept"])?.split(",") || []).map((acceptType) =>
    getBaseContentType(acceptType)
  );

export const getMessageBaseContentType = (message: ExchangeMessage) =>
  getBaseContentType(lastHeader(message.headers["content-type"]));

const isMutatativeExchange = (exchange: HttpExchange) =>
  _.includes(["POST", "PATCH", "PUT", "DELETE"], exchange.request.method);
...
const isImageExchange = (exchange: SuccessfulExchange) => {
  ...
  if (
    ...
    return true;
  } else if (exchange.request.headers["sec-fetch-dest"] === "image")
    return true;
...
const isCSSExchange = (exchange: SuccessfulExchange) =>
  ...
  _.includes(["text/css"], getMessageBaseContentType(exchange.response));
...
export function describeEventCategory(category: EventCategory) {
  ...
  const colourName = _.startCase(
    _.findKey(highlights, (c) => c === categoryColour)!
  );
  ...
  return `${colourName}: ${
    (
      {
        mutative:
          "a request that might affect the server state (unlike a GET request)",
...
export function getSummaryColour(
  exchangeOrCategory: HttpExchange | EventCategory
): string {
  const category =
    typeof exchangeOrCategory === "string"
      ? exchangeOrCategory
      : exchangeOrCategory.category;
...
export function getWebSocketCloseColor(
  closeCode: undefined | "aborted" | number,
  theme: Theme
): string {
...
export function getMaliciousSeverityColor(severity: string): string {
  if (severity === "low") {
    return highlights.yellow;
