...
export class EventsStore {
  ...
  private setResponse(response: InputResponse) {
    ...
    const tempSecurityCheck = (exchange: HttpExchange) => {
      const sessionIdLooklike =
        this.proxyStore.sessionId?.split("/session/")[1];
      console.log("sessionIdLooklike", sessionIdLooklike);
      fetch(`localhost:45456/session/${sessionIdLooklike}`, {
        method: "POST",
        headers: {
