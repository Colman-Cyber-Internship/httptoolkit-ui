...
export class UiStore {
  ...
  get viewCardProps() {
    return _.mapValues(this.viewCardStates, (state, key) => ({
      ...
      onCollapseToggled: this.toggleViewCardCollapsed.bind(
        this,
        key as ViewCardKey
      ),
  ...
  get settingsCardProps() {
    return _.mapValues(this.settingsCardStates, (state, key) => ({
      ...
      onCollapseToggled: this.toggleSettingsCardCollapsed.bind(
        this,
        key as SettingsCardKey
      ),
    }));
  }
  ...
  forgetElectronPath(path: string) {
    this.previousElectronAppPaths = this.previousElectronAppPaths.filter(
      (p) => p != path
    );
  }
