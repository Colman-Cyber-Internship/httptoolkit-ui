...
describe("Docker CLI interception", function () {
  ...
  ["JS", "Java", "Python", "Ruby", "Go"].forEach((target) => {
    it(`should intercept 'docker run' of local ${target} containers`, async function () {
      ...
      const { exitCode: buildExitCode, stdout: buildStdout } =
        await spawnToResult(
          "docker",
          ["build", ".", "-q"],
          {
      ...
      const { server, httpsConfig } = await testSetup;
      const mainRule = await server
        .forGet("https://example.test")
        .thenReply(200);

      const terminalEnvOverrides = getTerminalEnvVars(
        server.port,
        httpsConfig,
        process.env,
        {}
      ...
      childProc.spawn(
        "docker",
        ["run", "--rm", imageId, "https://example.test"],
        {
      ...
      expect(seenRequests.map((r) => r.url)).to.include(
        "https://example.test/"
      );
...
`
  ...
  it("should support proxying 'docker exec'", async function () {
    ...
    const runResult = await spawnToResult(
      "docker",
      ["run", "-d", "--rm", "node:14", "-e", "setTimeout(() => {}, 10000000)"],
      {
        env: { ...process.env, ...terminalEnvOverrides },
      }
    );
  ...
  it("should intercept 'docker-compose up'", async function () {
    ...
    const externalRule = await server
      .forAnyRequest()
      .forHost("example.test")
      .thenReply(200, "Mock response");
    const internalRule = await server.forUnmatchedRequest().thenPassThrough({
  ...
  it.skip("should intercept all network modes", async function () {
    ...
    const { exitCode, stdout } = await spawnToResult(
      "docker-compose",
      ["-f", "docker-compose.networks.yml", "up"],
      {
        env: { ...process.env, ...terminalEnvOverrides },
        cwd: composeRoot,
      },
  ...
  it("should clean up containers after shutdown", async () => {
    ...
    const uninterceptedResult = await spawnToResult("docker", [
      "create",
      "node:14",
    ]);
    ...
    const docker = new Docker();
    const interceptedContainer = await docker
      .getContainer(interceptedContainerId)
      .inspect();
    expect(interceptedContainer.Config.Image).to.equal("node:14");

    const uninterceptedContainer = await docker
      .getContainer(uninterceptedContainerId)
      .inspect();
    ...
    const inspectResultAfterShutdown: unknown = await docker
      .getContainer(interceptedContainerId)
      .inspect()
      .catch((e) => e);
    expect(inspectResultAfterShutdown).to.be.instanceOf(Error);
    expect((inspectResultAfterShutdown as Error).message).to.include(
      "no such container"
    );

    // Unintercepted container is not touched:
    const uninterceptedContainerAfterShutdown = await docker
      .getContainer(uninterceptedContainerId)
      .inspect();
    expect(uninterceptedContainerAfterShutdown.Config.Image).to.equal(
      "node:14"
    );
  });
  ...
  it("should start & clean up tunnel automatically", async () => {
    ...
    await delay(500);
    let tunnel = await docker
      .getContainer(`httptoolkit-docker-tunnel-${server.port}`)
      .inspect()
      .catch((e) => e);
    ...
    tunnel = await docker
      .getContainer(`httptoolkit-docker-tunnel-${server.port}`)
      .inspect();
    expect(tunnel.Config.Image.split(":")[0]).to.equal(
