...
import { applySuggestionToText } from "../../../../src/model/filters/syntax-matching";
...
function createFilter(input: string): Filter {
  ...
  const updatedFilters = applySuggestionToFilters(
    initialFilters,
    suggestions[0]
  );
...
function getSuggestionDescriptions(input: string) {
  return getFilterSuggestions(FilterClasses, input).map((s) =>
    s.filterClass.filterDescription(input, s.matchType === "template")
  );
}
...
describe("Search filter model integration test:", () => {
  describe("Simple filter usage", () => {
    it("should suggest all filter names given no input", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index"))
      ).to.deep.equal([
  ...
  describe("Status filters", () => {
    ...
    it("should disambiguate status operators as they're entered", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index"))
      ).to.deep.equal([{ index: 6, showAs: ">{3-digit number}" }]);
    ...
    it("should complete a fully entered search filter", () => {
      let filters: FilterSet = [new StringFilter("status>=300")];
    ...
    it("should correctly filter for exact statuses", () => {
      ...
      const matchedEvents = exampleEvents.filter((e) =>
        statusFilter.matches(e)
      );
      expect(matchedEvents.length).to.equal(1);
      expect(
        (matchedEvents[0] as SuccessfulExchange).response.statusCode
      ).to.equal(404);
    });
    ...
    it("should correctly filter for filter ranges", () => {
      ...
      expect(
        (matchedEvents[1] as SuccessfulExchange).response.statusCode
      ).to.equal(301);
    });
    ...
    it("should show a more specific description given a complete operator", () => {
      ...
      expect(description).to.equal(
        "responses with a status greater than or equal to a given value"
      );
    });
    ...
    it("should show a basic description given a partial value", () => {
      ...
      expect(description).to.equal(
        "responses with a status greater than a given value"
      );
    });
    ...
    it("should show a fully specific description given a full input and exact value", () => {
      ...
      expect(description).to.equal("responses with status 201 (Created)");
    });
  ...
  describe("Error filters", () => {
    it("should correctly filter for error responses", () => {
      ...
      const exampleEvents = [
        ...
        getExchangeData({
          statusCode: 500,
          responseTags: ["passthrough-error:ECONNRESET"],
        }),
      ...
      expect((matchedEvents[1] as FailedTlsConnection).failureCause).to.equal(
        "cert-rejected"
      );
    });
  });
  ...
  describe("Method filters", () => {
    it("should correctly filter for the given method", () => {
      ...
      const matchedEvents = exampleEvents.filter((e) => filter.matches(e));
      expect(matchedEvents.length).to.equal(1);
      expect((matchedEvents[0] as SuccessfulExchange).request.method).to.equal(
        "POST"
      );
    });
  ...
  describe("Protocol filters", () => {
    it("should only suggest http for =http", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index", "matchType"))
      ).to.deep.equal([{ index: 9, showAs: "http", matchType: "full" }]);
    });
    ...
    it("should only suggest https for =https", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index", "matchType"))
      ).to.deep.equal([{ index: 9, showAs: "https", matchType: "full" }]);
    });
  ...
  describe("Hostname filters", () => {
    it("should correctly filter for a given exact hostname", () => {
      const filter = createFilter("hostname=pipe.tech");

      const exampleEvents = [
        getExchangeData({ responseState: "aborted" }),
        getExchangeData({ responseState: "pending" }),
        getExchangeData({ statusCode: 404 }),
        getExchangeData({ hostname: "pipe.tech", statusCode: 200 }),
        getExchangeData({ hostname: "pipe.tech:8080", statusCode: 200 }),
        getFailedTls(),
      ];
      ...
      expect(
        (matchedEvents[1] as SuccessfulExchange).request.parsedUrl.port
      ).to.equal("8080");
    });
    ...
    it("should correctly filter for a hostname starting component", () => {
      ...
      expect(matchedEvents.length).to.equal(2);
      expect(
        (matchedEvents[0] as SuccessfulExchange).request.parsedUrl.hostname
      ).to.equal("google.com");
      expect(
        (matchedEvents[1] as SuccessfulExchange).request.parsedUrl.hostname
      ).to.equal("google.es");
    });
  ...
  describe("Port filters", () => {
    it("should correctly filter for a given port", () => {
      ...
      const exampleEvents = [
        ...
        getExchangeData({ responseState: "pending" }),
        getExchangeData({ statusCode: 404 }),
        getExchangeData({ hostname: "pipe.tech:8080" }),
        getFailedTls(),
      ];
  ...
  describe("Path filters", () => {
    it("should correctly filter for a given path", () => {
      ...
      expect(matchedEvents.length).to.equal(1);
      expect(
        (matchedEvents[0] as SuccessfulExchange).request.parsedUrl.pathname
      ).to.equal("/home");
    });
    ...
    it("should correctly filter for a given path prefix", () => {
      ...
      expect(
        (matchedEvents[1] as SuccessfulExchange).request.parsedUrl.pathname
      ).to.equal("/home/missing");
    });
  });
  ...
  describe("Query filters", () => {
    ...
    it("should show a more specific description given a partial operator", () => {
      ...
      const [[templateSuggestion], [emptyQuerySuggestion]] = _.partition(
        suggestions,
        { matchType: "template" }
      );

      const templateDescription =
        templateSuggestion.filterClass.filterDescription(
          input,
          templateSuggestion.matchType === "template"
        );

      const emptyQueryDescription =
        emptyQuerySuggestion.filterClass.filterDescription(
          input,
          emptyQuerySuggestion.matchType === "template"
        );
  ...
  describe("Headers filters", () => {
    ...
    it("should show descriptions for various suggestions", () => {
      [
        ...
        [
          "headers*=",
          "exchanges with any header value containing a given string",
        ],
        [
    ...
    it("should correctly filter the value of a header", () => {
      ...
      const matchedHeaders = (matchedEvents as HttpExchange[]).map((event) => ({
        ...
        ...(event.isSuccessfulExchange() ? event.response.headers : []),
      }));
  ...
  describe("Header filters", () => {
    it("should suggest header[...] in one step, once unambiguous", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index"))
      ).to.deep.equal([{ index: 0, showAs: "header[{header name}]" }]);
    });
  ...
  describe("Body filters", () => {
    ...
    it("should be able to filter for unicode content", async () => {
      ...
      const exampleEvents = [
        getFailedTls(),
        getExchangeData({
          requestBody: "an ascii string",
          responseBody: "more ascii",
        }),
        getExchangeData({
          requestBody: "NOT АЅСІІ",
          responseBody: "Ж Cyrillic chars",
        }),
        getExchangeData({
          responseState: "aborted",
          requestBody: "big-aborted-request",
      ...
      ).to.deep.equal(["NOT АЅСІІЖ Cyrillic chars"]);
    });
    ...
    it("should wait for and use decoded bodies", async () => {
      ...
      const exampleEvents = [
        getExchangeData({
          // gzipped correct match
          responseHeaders: { "content-encoding": "gzip" },
          responseBody: zlib.gzipSync("hello world"),
        }),
        getExchangeData({
          // This decodes but won't match
          responseHeaders: { "content-encoding": "gzip" },
      ...
      expect(
        matchedEvents.map((e: any) => e.response.body.decoded.toString())
  ...
  describe("Contains() filters", () => {
    it("should correctly filter across all properties", async () => {
      ...
      const nonMatchedEvents = exampleEvents
  ...
  describe("Or() filters", () => {
    ...
    it("should suggest finishing or completing a pair of conditions", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index", "matchType"))
      ).to.deep.equal([{ index: 14, showAs: "errored)", matchType: "full" }]);
    });
    ...
    it("should show descriptions for various suggestions", () => {
      [
        ...
        [
          "or(errored,",
          "requests that weren't transmitted successfully, or ...",
        ],
        [
          "or(errored, ",
          "requests that weren't transmitted successfully, or ...",
        ...
        [
          "or(errored,method",
          "requests that weren't transmitted successfully, or requests with a given method",
        ],
        [
          "or(errored, method",
          "requests that weren't transmitted successfully, or requests with a given method",
        ...
        [
          "or(errored, method=POST",
          "requests that weren't transmitted successfully, or POST requests",
        ],
        [
          "or(errored, method=POST, ",
          "requests that weren't transmitted successfully, POST requests, or ...",
        ],
        [
          "or(errored, method=POST)",
          "requests that weren't transmitted successfully, or POST requests",
        ...
        [
          "or(errored, path=/X)",
          "requests that weren't transmitted successfully, or requests to /X",
        ],
      ].forEach(([input, expectedOutput]) => {
        const description = getSuggestionDescriptions(input)[0];
        expect(description).to.equal(expectedOutput);
  ...
  describe("Not() filters", () => {
    ...
    it("should suggest fully completing a completable inner filter", () => {
      ...
      expect(
        suggestions.map((s) => _.pick(s, "showAs", "index", "matchType"))
      ).to.deep.equal([{ index: 4, showAs: "completed)", matchType: "full" }]);
    });
    ...
    it("should show descriptions for various suggestions", () => {
      [
        ...
        [
          "not(error",
          "excluding requests that weren't transmitted successfully",
        ],
        ["not(head", "excluding exchanges by all header values"],
        [
          "not(query^=?abc)",
          "excluding requests with a query string starting with ?abc",
        ],
