...
module.exports = function reconfigureElectron(params) {
  ...
  wrapModule(
    "electron",
    function wrapElectron(loadedModule) {
      if (electronWrapped || !loadedModule.app || !loadedModule.app.commandLine)
        return;
      ...
      app.commandLine.appendSwitch(
        ...
        params.spkiFingerprint
      );

      app.on("quit", () => {
        // This means the user has exited the app whilePipe is still running. That's fine,
        ...
        try {
          ...
          console.log(
            "Could not disconnect app via inspector, killing manually",
            e
          );
      ...
      app.on("ready", () => {
        loadedModule.session.defaultSession.setCertificateVerifyProc(
          (req, callback) => {
            if (
              req.certificate &&
